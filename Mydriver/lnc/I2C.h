...
#define Check_PClock_Value(__pClock__ , __I2C_Clock__)       ( (__I2C_Clock__ <= I2C_SPEED_Standart) ? (__pClock__ <= 20000000U) : (__pClock__ <= 40000000U) )
...
typedef struct {
		...
		uint32_t Addressing_Mode;  // I2C Address Mode Choice @def_group I2C_ADDR_Mode
		uint32_t DutyCycle;		   // I2C Duty Mode Choice @def_group I2C_Duty_Mode
		uint32_t MyOwnAddress;     // I2C Slave Mode
...
typedef struct {
	...
	I2C1_Init_Typedef_t Init;
	I2C_TypeDef_t *Instance;


}I2C1_HandleTypedef_t;
